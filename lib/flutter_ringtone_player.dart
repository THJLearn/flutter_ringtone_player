...
class FlutterRingtonePlayer {
  ...
  static Future<void> play(
    ...
    if (fromAsset == null) {
      ...
      fromAsset = await _generateAssetUri(fromAsset, isOriginalPath);
    }
    try {
  ...
  static Future<void> playAndroidUri({String? uriPath}) async {
    if (uriPath == null) {
      throw "Please specify the sound source.";
    }
    ...
    try {
      var args = <String, dynamic>{};
      args['uriPath'] = uriPath;
      _channel.invokeMethod('playWithUri', args);
    } on PlatformException {}
  }
  ...
  static Future<String> _generateAssetUri(
      String asset, bool? isOriginalPath) async {
    if (Platform.isAndroid) {
      if (isOriginalPath == true) return asset;
